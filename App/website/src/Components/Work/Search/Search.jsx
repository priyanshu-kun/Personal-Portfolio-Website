    ...
    return (
        <div className={`w-4/5 mt-4 absolute z-10 flex flex-col h-96 justify-start sm:max-w-168 sm:left-1/2 sm:top-0 sm:transform sm:-translate-x-2/4 ${!searchComp && "pointer-events-none"}`}>
            <div className={`h-14 sm:h-14 relative search-bar ${searchComp && "active"}`}>
                <input onChange={handleInputData} value={wordEntered} className="text-tiny work-search-bar inline-block absolute h-full w-full px-6 shadow-lg rounded-xl text-black outline-none sm:text-base" type="text" placeholder="Search for something!" />
                        ...
                        SearchableData.length > 0 ? (
                            SearchableData.slice(0, 3).map(obj => {
                                return <div onClick={() => {
                                    ...
                                    <h1 className="font-secondaryHeading text-base mb-2">* {obj.title.split("-")[0]}</h1>
                                    <p className={`search-item ${searchComp && "search-item-active"} text-tiny ml-3 opacity-30`}>{obj.title}</p>
                                </div>
                            })
                        ) : (
