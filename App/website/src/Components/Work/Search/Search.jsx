...
import "./Search.css"
 ...
 export default function Search({ searchComp, handleSearchComponent }) {
    ...
    return (
        <div className={`w-4/5 mt-4 absolute z-nv-1 flex flex-col h-96 justify-end overflow-hidden`}>
            <div className={`h-12 relative search-bar  ${searchComp && "active"}`}>
                <input className="text-tiny work-search-bar inline-block absolute h-full w-full px-6 shadow-lg rounded-lg text-black outline-none" type="text" placeholder="Search for something!" />
                <button onClick={handleSearchBarClose} className="absolute right-2 top-2/4 transform -translate-y-1/2 z-10"><img src={X} /></button>
            </div>
            <div className={`search-list-bar w-full h-80 bg-txt-primary-dark-bg mt-2 overflow-y-scroll overflow-x-hidden rounded-xl  shadow-lg ${searchComp && "search-list-active"} `}>
